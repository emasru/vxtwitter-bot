    ...
    async def callback(self, interaction):
        ...
        for replacement_pair in LINK_REPLACEMENTS:
            if replacement_pair[1] in interaction.message.content:
                ...
                await interaction.message.content.edit(content=reverted_content, view=d.ui.View(delete, timeout=None))
                break
...
def reply_to_post(message: d.Message) -> bool:
    ...
    mentioned_user: Union[d.User, d.Member] = reply_unknown.author # type: ignore TODO
    ...
    not_found = True
    for replacement_pair in LINK_REPLACEMENTS:
        if replacement_pair[1] in reply_unknown.content:
            not_found = False
            break
    if not_found:
        return False
